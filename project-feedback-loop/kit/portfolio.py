...
def main() -> None:
    ...
    payload = {"schema_version": 1, "generated_at": generated_at, "workspaces": workspace_names, "markers": markers_by_workspace, "excluded_dirs": excluded_dirs_by_workspace, "observed_at": observed_at, "items": items}
    ...
    workspace_text = html.escape(", ".join(workspace_names))
    marker_text = html.escape("; ".join(f"{name}: {', '.join(markers_by_workspace[name]) or 'none'}" for name in workspace_names))
    ...
    (args.output / "manifest.json").write_text(json.dumps({"schema_version": 1, "generated_at": generated_at, "workspaces": workspace_names, "artifacts": ["portfolio.json", "portfolio.html"]}, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    print(args.output)
